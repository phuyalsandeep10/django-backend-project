...
async def get_sla(sla_id: int, user=Depends(get_current_user)):
...
async def get_sla(
    sla_id: int, payload: EditTicketSLASchema, user=Depends(get_current_user)
):
...
async def delete_sla(sla_id: int, user=Depends(get_current_user)):
