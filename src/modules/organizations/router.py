...
from src.common.utils import generate_numeric_token
...
def invite_user(body:OrganizationInviteDto, user=Depends(get_current_user)):
    ...
    record = OrganizationInvitation.create(email=body.email,invited_by_id=user.id,token=token)
...
def reject_invitation(invitation_id:int,user=Depends(get_current_user)):
    ...
    for role_id in invitation.role_ids:
        OrganizationMember.create(role_id=role_id,organization_id=invitation.organization_id)
